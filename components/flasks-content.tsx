...
export function FlasksContent() {
  ...
  const [alcoholInventory, setAlcoholInventory] = useState<any>([])
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: flasksData } = await supabase.from("flasks").select("*");
      ...
      const lowStockFlasks = flasksData?.filter((f) => f.current_stock <= f.min_stock).length || 0
      ...
      const totalValue = flasksData?.reduce((sum, f) => sum + (f.cost_per_unit || 0), 0) || 0
  ...
  const filteredFlasks = flasks.filter(
    (flask : any) =>
      flask.material.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {filteredFlasks.map((flask: any) => (
                    <TableRow key={flask.id}>
                      <TableCell>
                        <div>
                          <div className="font-medium">{flask.flask_type_name}</div>
                          <Badge variant={getCategoryColor(flaskTypes.find((type : any) => type.id === flask.flask_type_id ).category)} className="text-xs">
                            {
                            flaskTypes.find((type : any) => type.id === flask.flask_type_id ).category
                            }    
                          </Badge>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          ...
                          {flask.current_stock <= flask.min_stock && <AlertTriangle className="h-4 w-4 text-red-500" />}
                        </div>
                        <div className="text-sm text-muted-foreground">Mín: {flask.min_stock}</div>
                      </TableCell>
                      <TableCell>
                        <div className="font-medium">${flask.cost_per_unit?.toFixed(2) || "0.00"}</div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">{flask.location || "N/A"}</div>
                      </TableCell>
                      <TableCell>
                        <Badge variant={getStockStatusColor(flask.status)}>{flask.status}</Badge>
                      </TableCell>
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {filteredAlcohol.map((alcohol: any) => (
